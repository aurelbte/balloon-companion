...
export async function GET(request: Request) {
  ...
  if (latitude === null || longitude === null || !isValidCoordinate({ latitude, longitude }) || !weatherModel || !OPEN_METEO_WEATHER_MODELS.includes(weatherModel)) return Response.json({ error: { code: "INVALID_REQUEST", message: "lat, lon et weatherModel supporté sont requis." } }, { status: 400 });
