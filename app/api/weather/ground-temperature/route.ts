...
export async function GET(request: Request) {
  ...
  const timeZone = requestedTimeZone && isValidTimeZone(requestedTimeZone) ? requestedTimeZone : "UTC";
  ...
  try {
    const raw = await createOpenMeteoClient(getOpenMeteoServerConfig()).fetchGroundTemperature({ latitude, longitude, validAt: selectedValidAt, weatherModel: "best_match", timeZone }) as TemperaturePayload;
