...
import { refreshStorageEstimate, storageEstimateWarning } from "../../../../../../lib/storageResilience";
...
function AddBalloonDocumentContent() {
  ...
  const chooseFile = (selected: File | undefined) => { if (!selected) return; const issue = validateBalloonDocumentFile(selected); if (issue) { setError(issue); return; } void refreshStorageEstimate(); setFile(selected); setError(null); if (!title.trim()) setTitle(documentTitleFromFileName(selected.name)); };
  ...
  return <main className={styles.screen}><div className={styles.layout}><Link href={`/more/profile/balloons/${encodeURIComponent(balloonId)}/documents`} className={styles.back}><ChevronLeft size={18} /> Documents</Link><header><p className={styles.eyebrow}>Porte-documents</p><h1 className={styles.title}>Ajouter un document</h1></header>
    <form className={styles.documentForm} onSubmit={(event) => { event.preventDefault(); void save(); }}>
      ...
      {file && storageEstimateWarning(storageDiagnostics.estimate, file.size) && <p role="status" className={styles.subtitle}>{storageEstimateWarning(storageDiagnostics.estimate, file.size)}</p>}
