...
import { calculateCommercialQualification } from "../../../lib/commercialQualificationEngine";
...
function commercialMaintenanceRoute(event: QualificationEvent | undefined): string {
  if (event?.type === "INITIAL_COMMERCIAL_ISSUANCE") return "Délivrance initiale";
  if (event?.type === "COMMERCIAL_PROFICIENCY_CHECK") return "Contrôle de compétences FE(B)";
  if (event?.type === "COMMERCIAL_REFRESHER_COURSE") return "Formation / remise à niveau";
  if (event?.type === "OPERATOR_PROFICIENCY_CHECK") return "Crédit par contrôle opérateur";
  return "Aucune voie de maintien admissible enregistrée";
}
...
function QualificationSettingsForm({ priority = false, settings, onChange, onSubmit }: {
  ...
  const toggleCommercialClass = (classId: "HOT_AIR_BALLOON" | "GAS_BALLOON", checked: boolean) => onChange({
    ...settings,
    commercialBalloonClasses: checked ? [...settings.commercialBalloonClasses, classId] : settings.commercialBalloonClasses.filter((value) => value !== classId),
  });
  return <form className={`${styles.settings} ${priority ? styles.prioritySettings : ""}`} onSubmit={onSubmit}>
    ...
    <div className={styles.settingsGrid}>
      ...
      <label className={styles.toggle} htmlFor="qualification-commercial"><input id="qualification-commercial" type="checkbox" checked={settings.commercialOperationsEnabled} onChange={(event) => onChange({ ...settings, commercialOperationsEnabled: event.target.checked })} /><span>Activité commerciale passagers</span></label>
      {settings.commercialOperationsEnabled && <div className={styles.privileges}><span>CLASSES COMMERCIALES DÉTENUES</span><label className={styles.toggle}><input type="checkbox" checked={settings.commercialBalloonClasses.includes("HOT_AIR_BALLOON")} onChange={(event) => toggleCommercialClass("HOT_AIR_BALLOON", event.target.checked)} /><span>Ballon à air chaud</span></label><label className={styles.toggle}><input type="checkbox" checked={settings.commercialBalloonClasses.includes("GAS_BALLOON")} onChange={(event) => toggleCommercialClass("GAS_BALLOON", event.target.checked)} /><span>Ballon à gaz</span></label></div>}
      {settings.commercialOperationsEnabled && settings.commercialBalloonClasses.includes("HOT_AIR_BALLOON") && <label className={styles.privilegeGroup}><span>Groupe hot-air commercial maximal détenu</span><select value={settings.commercialHotAirBalloonGroupPrivilege ?? ""} onChange={(event) => onChange({ ...settings, commercialHotAirBalloonGroupPrivilege: (["A", "B", "C", "D"].includes(event.target.value) ? event.target.value : null) as QualificationProfile["commercialHotAirBalloonGroupPrivilege"] })}><option value="">Non renseigné</option><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select></label>}
...
function CommercialEventForm({ type, draft, editing, trainingEvents, error, onChange, onCancel, onDelete, onSubmit }: { type: EditableCommercialEventType; draft: CommercialEventDraft; editing: boolean; trainingEvents: readonly QualificationEvent[]; error: string; onChange: (draft: CommercialEventDraft) => void; onCancel: () => void; onDelete?: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const check = type === "COMMERCIAL_PROFICIENCY_CHECK" || type === "OPERATOR_PROFICIENCY_CHECK";
  const title = issuance ? "Délivrance initiale — activité commerciale passagers" : type === "OPERATOR_PROFICIENCY_CHECK" ? "Contrôle de compétences opérateur" : check ? "Contrôle de compétences commercial" : "Formation / remise à niveau commerciale";
  return <form className={styles.eventForm} onSubmit={onSubmit}>
    ...
    {check && <label className={styles.eventFormWide}><span>{type === "OPERATOR_PROFICIENCY_CHECK" ? "Examinateur" : "Examinateur FE(B)"}</span><input required value={draft.personName} onChange={(event) => onChange({ ...draft, personName: event.target.value })} /></label>}
    {type === "COMMERCIAL_REFRESHER_COURSE" && <><label><span>Durée théorique (minutes)</span><input required inputMode="numeric" type="number" min="360" value={draft.theoryMinutes} onChange={(event) => onChange({ ...draft, theoryMinutes: event.target.value })} /></label><label><span>Vol associé avec FI(B)</span><select required value={draft.trainingEventId} onChange={(event) => onChange({ ...draft, trainingEventId: event.target.value })}><option value="">Choisir</option>{trainingEvents.filter((event) => event.balloonClass?.classId === draft.classId && (draft.classId !== "HOT_AIR_BALLOON" || event.balloonClass.groupId === draft.groupId)).map((event) => <option key={event.id} value={event.id}>{formatQualificationDate(event.dateIso)} · {event.instructor?.name}</option>)}</select></label><label className={styles.toggle}><input type="checkbox" checked={draft.commercialQualifiedFiB} onChange={(event) => onChange({ ...draft, commercialQualifiedFiB: event.target.checked })} /><span>FI(B) qualifié pour l’activité commerciale</span></label></>}
    <label className={styles.eventFormWide}><span>Notes (facultatif)</span><textarea value={draft.notes} onChange={(event) => onChange({ ...draft, notes: event.target.value })} /></label>
...
function eventClassKeys(_events: readonly QualificationEvent[], _ascensions: PilotQualificationsPageState["completion"]["officialAscensions"], profile: QualificationProfile): QualificationBalloonClass[] {
  return profile.commercialBalloonClasses.map((classId) => ({ classId }));
}
...
export default function QualificationsPage() {
  ...
  const deleteCommercialDeclaration = (classId: string) => {
    if (!initialSituationDraft || !window.confirm("Supprimer cette déclaration initiale commerciale passagers ?")) return;
  ...
  const summary: Array<readonly [string, QualificationRequirementResult]> = [
    ...
    ...(qualifications.profile.commercialOperationsEnabled ? [["Activité commerciale passagers", commercialSummary] as const] : []),
  ];
  const actionItems: Array<Readonly<{ text: string; href?: string; initialSituation?: boolean; editor?: EditableQualificationEventType; bplType?: EditableBplEventType; commercialType?: EditableCommercialEventType; commercialClassId?: string; buttonLabel?: string }>> = [
    ...
    ...(qualifications.profile.commercialOperationsEnabled && view.commercial.length === 0 ? [{ text: "Choisir vos classes commerciales détenues", buttonLabel: "Modifier" }] : []),
  ...
  return <main className={moreStyles.screen}><div className={moreStyles.layout}>
    ...
    {qualifications.profile.commercialOperationsEnabled && <section className={styles.section} aria-labelledby="commercial-title"><div className={styles.sectionHeader}><h2 id="commercial-title">Activité commerciale passagers</h2><StatusBadge status={commercialSummary.status} /></div>
      {view.commercial.length === 0 ? <p className={styles.compactText}>Choisissez au moins une classe commerciale détenue dans « Modifier ma situation ».</p> : view.commercial.map((commercial) => { const classId = commercial.balloonClass.classId; const issuance = commercialIssuances.find((event) => event.balloonClass?.classId === classId); const check = qualifications.events.find((event) => event.type === "COMMERCIAL_PROFICIENCY_CHECK" && event.balloonClass?.classId === classId); const operatorCheck = qualifications.events.find((event) => event.type === "OPERATOR_PROFICIENCY_CHECK" && event.balloonClass?.classId === classId); const course = qualifications.events.find((event) => event.type === "COMMERCIAL_REFRESHER_COURSE" && event.balloonClass?.classId === classId); const maintenanceEvent = qualifications.events.find((event) => commercial.maintenance.sourceEventIds?.includes(event.id) && ["INITIAL_COMMERCIAL_ISSUANCE", "COMMERCIAL_PROFICIENCY_CHECK", "COMMERCIAL_REFRESHER_COURSE", "OPERATOR_PROFICIENCY_CHECK"].includes(event.type)); const recencyValue = commercial.recency.currentValue as { picFlights?: number; flightsInClass?: number; supervisedFlightsInClass?: number } | undefined; return <article className={styles.commercialClass} key={classId}><h3>{qualificationClassLabel(classId)}</h3>
        ...
        <CompactRequirement title="Récence — 180 jours" result={commercial.recency}><p>{recencyValue?.picFlights ?? 0} vols PIC acquis · {recencyValue?.flightsInClass ?? 0} dans cette classe</p>{commercial.recency.status === "UNKNOWN" && <><p>Historique BC incomplet sur les 180 derniers jours. Ajoutez vos vols antérieurs ou confirmez votre situation initiale.</p><Link className={styles.historyLink} href="/journal/ascension/new">Ajouter un vol antérieur →</Link><button className={styles.inlineAction} type="button" onClick={openInitialSituation}>Confirmer mon historique antérieur</button></>}</CompactRequirement>
        <CompactRequirement title="Maintien commercial — 24 mois" result={commercial.maintenance}><p>Voie retenue : {commercialMaintenanceRoute(maintenanceEvent)}{commercial.maintenance.dueDate ? ` · échéance ${formatQualificationDate(commercial.maintenance.dueDate)}` : ""}</p><button className={styles.inlineAction} type="button" onClick={() => openCommercialEditor("COMMERCIAL_PROFICIENCY_CHECK", check, classId)}>{check ? "Modifier le contrôle FE(B)" : "Ajouter un contrôle FE(B)"}</button><button className={styles.inlineAction} type="button" onClick={() => openCommercialEditor("COMMERCIAL_REFRESHER_COURSE", course, classId)}>{course ? "Modifier la formation" : "Ajouter une formation"}</button></CompactRequirement>
        <CompactRequirement title="Contrôle opérateur — 24 mois" result={commercial.operatorEquivalent}><button className={styles.inlineAction} type="button" onClick={() => openCommercialEditor("OPERATOR_PROFICIENCY_CHECK", operatorCheck, classId)}>{operatorCheck ? "Modifier" : "Ajouter"}</button></CompactRequirement>
        {commercial.groupLimitation && <p className={styles.compactText}>Groupe commercial détenu : {commercial.groupLimitation.heldGroup ?? "non renseigné"} · Groupe actuellement exerçable : {commercial.groupLimitation.exercisableGroup ? `A à ${commercial.groupLimitation.exercisableGroup}` : "non déterminable"}</p>}
      </article>; })}
