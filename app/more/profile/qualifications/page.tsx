...
export default function QualificationsPage() {
  ...
  return <main className={moreStyles.screen}><div className={moreStyles.layout}>
    ...
    {qualifications.profile.commercialOperationsEnabled && <section className={styles.section} aria-labelledby="commercial-title"><div className={styles.sectionHeader}><h2 id="commercial-title">Activité commerciale passagers</h2><StatusBadge status={commercialSummary.status} /></div>
      {view.commercial.length === 0 ? <p className={styles.compactText}>Choisissez au moins une classe commerciale détenue dans « Modifier ma situation ».</p> : view.commercial.map((commercial) => { const classId = commercial.balloonClass.classId; const issuance = commercialIssuances.find((event) => event.balloonClass?.classId === classId); const check = qualifications.events.find((event) => event.type === "COMMERCIAL_PROFICIENCY_CHECK" && event.balloonClass?.classId === classId); const operatorCheck = qualifications.events.find((event) => event.type === "OPERATOR_PROFICIENCY_CHECK" && event.balloonClass?.classId === classId); const course = qualifications.events.find((event) => event.type === "COMMERCIAL_REFRESHER_COURSE" && event.balloonClass?.classId === classId); const recencyValue = commercial.recency.currentValue as { picFlights?: number; flightsInClass?: number; supervisedFlightsInClass?: number } | undefined; return <article className={styles.commercialClass} key={classId}><h3>{qualificationClassLabel(classId)}</h3>
        ...
        <CompactRequirement title="Maintien 24 mois — contrôle de compétences" result={commercial.proficiencyCheckFeB}><button className={styles.inlineAction} type="button" onClick={() => openCommercialEditor("COMMERCIAL_PROFICIENCY_CHECK", check, classId)}>{check ? "Modifier" : "Ajouter"}</button></CompactRequirement>
        <CompactRequirement title="Maintien 24 mois — formation / remise à niveau" result={commercial.refresherCourse}><button className={styles.inlineAction} type="button" onClick={() => openCommercialEditor("COMMERCIAL_REFRESHER_COURSE", course, classId)}>{course ? "Modifier" : "Ajouter"}</button></CompactRequirement>
        <CompactRequirement title="Maintien 24 mois — contrôle opérateur" result={commercial.operatorEquivalent}><button className={styles.inlineAction} type="button" onClick={() => openCommercialEditor("OPERATOR_PROFICIENCY_CHECK", operatorCheck, classId)}>{operatorCheck ? "Modifier" : "Ajouter"}</button></CompactRequirement>
