...
import { loadAviationPreferences, saveAviationPreferences } from "../lib/aviation/aviationPreferencesStorage";
...
export default function WeatherPage() {
  ...
  const [aviationAirportDraft, setAviationAirportDraft] = useState("");
  ...
  useEffect(() => { if (aviationPreferencesReady) return; const stored = loadAviationPreferences(); if (!stored && preferences.favorites.length === 0) return; const airport = stored?.airportIcao ?? normalizeAirportIcao(preferences.activeFavorite?.icaoCode); if (!stored) saveAviationPreferences(airport); setAviationAirport(airport); setAviationAirportDraft(airport ?? ""); setAviationPreferencesReady(true); }, [aviationPreferencesReady, preferences.activeFavorite, preferences.favorites.length]);
