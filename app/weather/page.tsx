...
function AviationAirportPicker({ favorites, selected, onChange }: { favorites: readonly AviationAirportFavorite[]; selected: string | null; onChange(next: { selected: string | null; favorites: AviationAirportFavorite[] }): void }) {
  ...
  const [results, setResults] = useState<AviationAirportFavorite[]>([]);
  const [searching, setSearching] = useState(false);
  const search = async () => { if (query.trim().length < 2) return; setSearching(true); try { const response = await fetch(`/api/aviation/airports/search?q=${encodeURIComponent(query.trim())}`); const payload = await response.json() as { results?: AviationAirportFavorite[] }; setResults(payload.results ?? []); } finally { setSearching(false); } };
  ...
  return <section className={`${styles.card} ${styles.preferenceGroup}`}><div className={styles.preferenceHeading}><span className={styles.eyebrow}>Aérodrome Aviation</span><div className={styles.aviationActions}><button type="button" onClick={() => setAdding(true)}><Plus size={14} />Ajouter</button><button type="button" onClick={() => setEditing((value) => !value)}>Modifier</button></div></div><div className={styles.chips}>{favorites.length === 0 ? <span className={styles.empty}>Aucun aérodrome Aviation sélectionné</span> : favorites.map((favorite) => <span className={styles.editableChip} key={favorite.icao}><button type="button" aria-pressed={favorite.icao === selected} onClick={() => onChange({ selected: favorite.icao, favorites: [...favorites] })}>{favorite.icao}</button>{editing && <button type="button" aria-label={`Supprimer ${favorite.icao}`} onClick={() => remove(favorite.icao)}><Trash2 size={13} /></button>}</span>)}</div>{adding && <div className={styles.airportSearch}><div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ICAO ou nom de l’aérodrome" autoFocus /><button type="button" onClick={search}>{searching ? "Recherche…" : "Rechercher"}</button></div>{results.map((result) => <button type="button" key={result.icao} onClick={() => add(result)}><strong>{result.icao}</strong><span>{result.name}</span></button>)}<button type="button" onClick={() => setAdding(false)}>Fermer</button></div>}</section>;
}
