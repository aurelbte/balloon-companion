...
type PassengerMemoryDialogProps = {
  ...
  recordedFlightBalloonRegistration?: string;
  activeBalloonId: string | null;
  onCancel: () => void;
  onCreate: (displayedDuration: string, balloonId: string) => Promise<void>;
};

export default function PassengerMemoryDialog({ defaultDuration, balloons, recordedFlightBalloonRegistration, activeBalloonId, onCancel, onCreate }: PassengerMemoryDialogProps) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const [duration, setDuration] = useState(defaultDuration);
  const [balloonId, setBalloonId] = useState(() => defaultPassengerMemoryBalloonId(balloons, recordedFlightBalloonRegistration, activeBalloonId));
