...
import { pilotFunctionForRegulatoryRole, roundJournalAltitudeMeters, type OfficialAscensionInput, type OfficialFlightNature, type OfficialRegulatoryRole } from "../../lib/flightCompletion";
...
export type OfficialAscensionFormValues = {
  ...
  regulatoryRole: OfficialAscensionInput["regulatoryRole"];
  supervisedByFiB: OfficialAscensionInput["supervisedByFiB"];
...
export default function OfficialAscensionForm({ mode, ascensionId, title, subtitle, backLabel, submitLabel, gpsDurationMinutes, manualDateEntry = false, nativeSubmit = false, initialValues, onCancel, onSubmit }: Props) {
  ...
  const supervisedPic = values.regulatoryRole === "PIC" && values.supervisedByFiB === true;
  const validPeople = (!(flightNatureRequiresInstructor(values.flightNature) || supervisedPic) || Boolean(values.instructorName.trim())) && (!flightNatureRequiresExaminer(values.flightNature) || Boolean(values.examinerName.trim()));
  const validRole = values.regulatoryRole !== null || mode === "EDIT" && Boolean(values.pilotFunction);
  const valid = Boolean(values.dateIso && values.balloonModel.trim() && values.registration.trim() && values.departure.trim() && values.arrival.trim() && values.category && values.pilotFunction && validRole && Number.isInteger(duration) && duration > 0 && validAltitude && validMovements && validPeople);
  ...
  const chooseRegulatoryRole = (role: OfficialRegulatoryRole) => {
    setValues((current) => ({ ...current, regulatoryRole: role, pilotFunction: pilotFunctionForRegulatoryRole(role), supervisedByFiB: false }));
    setDirty(true);
  };
  ...
  return <main className={styles.screen}><div className={styles.layout}>
    ...
    <form id="official-ascension-form" className={styles.form} onSubmit={(event) => { event.preventDefault(); void submit(); }} onKeyDown={moveToNextField}>
      ...
      <label><span>Fonction</span><select value={values.regulatoryRole ?? (mode === "EDIT" && values.pilotFunction ? "LEGACY" : "")} onChange={(e) => { if (e.target.value && e.target.value !== "LEGACY") chooseRegulatoryRole(e.target.value as OfficialRegulatoryRole); }}><option value="">Sélectionner</option>{mode === "EDIT" && values.regulatoryRole === null && values.pilotFunction && <option value="LEGACY">{values.pilotFunction} — fonction réglementaire à préciser</option>}<option value="PIC">Commandant de bord (PIC)</option><option value="DUAL">Double commande</option><option value="FI_B">Instructeur FI(B)</option><option value="FE_B">Examinateur FE(B)</option></select>{mode === "EDIT" && values.regulatoryRole === null && values.pilotFunction && <small>Fonction réglementaire à préciser</small>}</label>
      {values.regulatoryRole === "PIC" && <label className={styles.wide}><span><input type="checkbox" checked={values.supervisedByFiB === true} onChange={(e) => update("supervisedByFiB", e.target.checked)} /> Solo sous supervision FI(B)</span></label>}
      <label className={styles.wide}><span>Nature du vol</span><select value={values.flightNature} onChange={(e) => update("flightNature", e.target.value as OfficialFlightNature)}><option value="STANDARD">Vol standard</option><option value="CAPTIVE">Vol captif</option><option value="TRAINING_BPL">Vol d’entraînement BPL</option><option value="PROFICIENCY_CHECK_BPL">Contrôle de compétences BPL</option><option value="SKILL_TEST">Examen pratique</option><option value="COMMERCIAL_TRAINING">Formation commerciale</option><option value="COMMERCIAL_PROFICIENCY_CHECK">Contrôle de compétences commercial</option><option value="INSTRUCTION">Instruction</option></select></label>
      {(flightNatureRequiresInstructor(values.flightNature) || supervisedPic) && <><label><span>{supervisedPic ? "FI(B) superviseur — Nom" : "FI(B) — Nom"}</span><input required value={values.instructorName} onChange={(e) => update("instructorName", e.target.value)} /></label><label><span>{supervisedPic ? "FI(B) superviseur — N° de licence" : "FI(B) — N° de licence"} <small>(facultatif)</small></span><input autoCorrect="off" spellCheck={false} value={values.instructorLicenceNumber} onChange={(e) => update("instructorLicenceNumber", e.target.value)} /></label></>}
      {flightNatureRequiresExaminer(values.flightNature) && <><label><span>FE(B) — Nom</span><input required value={values.examinerName} onChange={(e) => update("examinerName", e.target.value)} /></label><label><span>FE(B) — N° de licence <small>(facultatif)</small></span><input autoCorrect="off" spellCheck={false} value={values.examinerLicenceNumber} onChange={(e) => update("examinerLicenceNumber", e.target.value)} /></label></>}
      <label><span>Décollages <small>(facultatif)</small></span><input inputMode="numeric" pattern="[0-9]*" value={values.takeoffCount} onChange={(e) => update("takeoffCount", e.target.value.replace(/\D/g, ""))} /></label><label><span>Atterrissages <small>(facultatif)</small></span><input inputMode="numeric" pattern="[0-9]*" value={values.landingCount} onChange={(e) => update("landingCount", e.target.value.replace(/\D/g, ""))} /></label>
