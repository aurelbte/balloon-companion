...
function HydratedGraphs({ flight }: { flight: JournalFlight }) {
  ...
  const speed = points.filter((point): point is typeof point & { speedKmh: number } => point.speedKmh !== null).map((point) => ({ x: point.elapsedMinutes, y: units.flightInstruments.speedUnit === "kt" ? kmhToKnots(point.speedKmh) : point.speedKmh }));
  ...
  return <main className={styles.screen}><div className={styles.layout}><Link href={`/journal/${flight.id}`} className={styles.backLink}>← Fiche de vol</Link><header className={styles.detailHeader}><p className={styles.eyebrow}>Graphiques</p><h1 className={styles.routeTitle}>{flight.departure} → {flight.arrival}</h1><p className={styles.dateLine}>{flight.date}</p></header><div className={styles.graphs}>{altitude.length > 1 ? <JournalChart title="Altitude" unit={`${units.flightInstruments.altitudeUnit} AMSL`} axisUnit={units.flightInstruments.altitudeUnit} color="var(--bc-color-navigation-strong)" yMaximum={Math.max(altitudeMinimum, Math.ceil(Math.max(...altitude.map(({ y }) => y)) / altitudeStep) * altitudeStep)} yStep={altitudeStep} points={altitude} /> : <p>Altitude indisponible</p>}{speed.length > 1 ? <JournalChart title="Vitesse sol" unit={units.flightInstruments.speedUnit} axisUnit={units.flightInstruments.speedUnit} color="var(--bc-color-launch)" yMaximum={Math.max(30, Math.ceil(Math.max(...speed.map(({ y }) => y)) / 5) * 5)} yStep={5} points={speed} /> : <p>Vitesse indisponible</p>}</div></div><NavigationBar activeItem="Journal" /></main>;
}
