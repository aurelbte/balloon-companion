...
function HydratedGraphs({ flight }: { flight: JournalFlight }) {
  ...
  return <main className={styles.screen}><div className={styles.layout}><Link href={`/journal/${flight.id}`} className={styles.backLink}>← Fiche de vol</Link><header className={styles.detailHeader}><p className={styles.eyebrow}>Graphiques</p><h1 className={styles.routeTitle}>{flight.departure} → {flight.arrival}</h1><p className={styles.dateLine}>{flight.date}</p></header><div className={styles.graphs}>{availableAltitudes.length > 1 ? <JournalChart title="Altitude" unit={`${units.flightInstruments.altitudeUnit} AMSL`} axisUnit={units.flightInstruments.altitudeUnit} color="var(--bc-color-navigation-strong)" yMaximum={Math.max(altitudeMinimum, Math.ceil(Math.max(...availableAltitudes) / altitudeStep) * altitudeStep)} yStep={altitudeStep} durationMinutes={durationMinutes} tooltipLabel="ALTITUDE" tooltipUnavailableLabel="Altitude indisponible" tooltipFractionDigits={0} tooltipTimePrefix="Temps de vol : " points={altitude} /> : <p>Altitude indisponible</p>}{availableSpeeds.length > 1 ? <JournalChart title="Vitesse sol" unit={units.flightInstruments.speedUnit} axisUnit={units.flightInstruments.speedUnit} color="var(--bc-color-launch)" yMaximum={Math.max(30, Math.ceil(Math.max(...availableSpeeds) / 5) * 5)} yStep={5} durationMinutes={durationMinutes} tooltipLabel="VITESSE SOL" tooltipUnavailableLabel="Vitesse indisponible" tooltipFractionDigits={1} points={speed} /> : <p>Vitesse indisponible</p>}</div></div><NavigationBar activeItem="Journal" /></main>;
}
