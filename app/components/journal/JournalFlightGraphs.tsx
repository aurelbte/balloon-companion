...
export default function JournalFlightGraphs({ flightId, initialFlight }: { flightId: string; initialFlight: JournalFlight | null }) {
  ...
  const altitude = flight.points.filter((point): point is typeof point & { altitudeM: number } => point.altitudeM !== null).map((point) => ({ x: point.elapsedMinutes, y: point.altitudeM }));
  const speed = flight.points.filter((point): point is typeof point & { speedKmh: number } => point.speedKmh !== null).map((point) => ({ x: point.elapsedMinutes, y: point.speedKmh }));
