...
export default function JournalFlightDetail({ flightId, initialFlight }: { flightId: string; initialFlight: JournalFlight | null }) {
  ...
  return <main id="flight-detail-top" className={`${styles.screen} ${styles.detailScreen}`}><div className={`${styles.layout} ${styles.detailLayout}`}>
    ...
    <JournalFlightMap key={flight.id} flight={flight} />
    ...
    <section className={styles.moduleGrid} aria-label="Informations du vol">
      ...
      <Link href={`/journal/${flight.id}/statistics`} className={`${styles.moduleCard} ${styles.moduleLink}`}><h2 className={styles.moduleTitle}><Gauge size={16} /> Statistiques</h2><div className={styles.statGrid}><p><span>Altitude max</span><strong>{flight.maxAltitudeM === null ? "—" : formatFlightAltitude(roundJournalAltitudeMeters(flight.maxAltitudeM)!, units.flightInstruments.altitudeUnit)}</strong></p><p><span>Vitesse max</span><strong>{flight.maxSpeedKmh === null ? "—" : formatFlightSpeed(flight.maxSpeedKmh, units.flightInstruments.speedUnit)}</strong></p></div><p className={styles.moduleAction}><span>Voir toutes les statistiques</span><ChevronRight size={15} /></p></Link>
      <article className={`${styles.moduleCard} ${styles.moduleLink}`} role="button" tabIndex={0} aria-label={displayedNote ? "Modifier la note de vol" : "Ajouter une note de vol"} onClick={() => setNoteEditorOpen(true)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setNoteEditorOpen(true); } }}><h2 className={styles.moduleTitle}><NotebookPen size={16} /> Notes</h2><p className={`${styles.moduleValue} ${styles.flightNotePreview}`}>{displayedNote ?? "Aucune note"}</p></article>
      <article className={`${styles.moduleCard} ${styles.moduleLink}`} role="button" tabIndex={0} aria-label="Ouvrir les options d’export du vol" onClick={() => setExportDialogOpen(true)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setExportDialogOpen(true); } }}><h2 className={styles.moduleTitle}><FileDown size={16} /> Export</h2><p className={`${styles.moduleValue} ${styles.exportFormats}`}>GPX · Souvenir · BCFLIGHT</p></article>
    </section>
  </div>{noteEditorOpen && <FlightNoteDialog initialNote={displayedNote ?? ""} onCancel={() => setNoteEditorOpen(false)} onSave={saveNote} />}{exportDialogOpen && <FlightExportDialog onClose={() => setExportDialogOpen(false)} onPreparePassengerMemory={async () => setPassengerMemoryFlight(await loadExportFlight())} onExportGpx={async () => { const recordedFlight = await loadExportFlight(); await exportGpx(recordedFlight, { date: flight.date, departure: flight.departure, arrival: flight.arrival }); }} onExportBcFlight={async () => { await exportBcFlight(await loadExportFlight()); }} />}{passengerMemoryFlight && <PassengerMemoryDialog defaultDuration={formatPassengerMemoryDuration(passengerMemoryFlight.summary.durationSeconds)} balloons={balloonState.registry.balloons} recordedFlightBalloonRegistration={passengerMemoryFlight.balloonRegistration} activeBalloonId={balloonState.registry.activeBalloonId} onCancel={() => setPassengerMemoryFlight(null)} onCreate={async (displayedDuration, balloonId) => { const selectedBalloon = balloonState.registry.balloons.find(({ id }) => id === balloonId); if (!selectedBalloon) throw new Error("Ballon introuvable"); await exportPassengerMemory({ recordedFlight: passengerMemoryFlight, journalFlight: flight, units: units.flightInstruments, displayedDuration, pilot: auth.user, selectedBalloon }); setPassengerMemoryFlight(null); }} />}<NavigationBar activeItem="Journal" /></main>;
}
