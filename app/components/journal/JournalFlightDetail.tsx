...
export default function JournalFlightDetail({ flightId, initialFlight }: { flightId: string; initialFlight: JournalFlight | null }) {
  ...
  return <main className={styles.screen}><div className={styles.layout}>
    ...
    <section className={styles.moduleGrid} aria-label="Informations du vol">
      ...
      <article className={styles.moduleCard}><h2 className={styles.moduleTitle}><NotebookPen size={16} /> Notes</h2><p className={`${styles.moduleValue} ${styles.flightNotePreview}`}>{displayedNote ?? "Aucune note"}</p><button type="button" className={styles.moduleTextAction} onClick={() => setNoteEditorOpen(true)}>{displayedNote ? "Modifier" : "Ajouter une note"}</button></article>
