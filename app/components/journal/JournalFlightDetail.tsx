...
export default function JournalFlightDetail({ flightId, initialFlight }: { flightId: string; initialFlight: JournalFlight | null }) {
  ...
  return <main id="flight-detail-top" className={`${styles.screen} ${styles.detailScreen}`}><div className={`${styles.layout} ${styles.detailLayout}`}>
    ...
    <section className={styles.moduleGrid} aria-label="Informations du vol">
      ...
      <article className={`${styles.moduleCard} ${styles.moduleLink}`} role="button" tabIndex={0} aria-label="Ouvrir les options d’export du vol" onClick={() => setExportDialogOpen(true)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setExportDialogOpen(true); } }}><h2 className={styles.moduleTitle}><FileDown size={16} /> Export</h2><p className={styles.moduleValue}>GPX · Souvenir · BCFLIGHT</p></article>
    </section>
