...
type RenameAscensionDialogProps = {
  initialName: string;
  onCancel: () => void;
  onConfirm: (name: string) => void;
};

export default function RenameAscensionDialog({
  initialName,
  onCancel,
  onConfirm,
}: RenameAscensionDialogProps) {
  ...
  return (
    <dialog
      ...
      aria-labelledby="rename-ascension-title"
      onCancel={(event) => {
        event.preventDefault();
        onCancel();
      }}
    >
      <h2 id="rename-ascension-title" className="text-xl font-semibold tracking-tight">Renommer l’ascension</h2>
      <label className="mt-4 block">
        <span className="sr-only">Nouveau titre personnel</span>
