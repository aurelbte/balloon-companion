...
test("associer une ascension d’entraînement réutilise le mapping et ne duplique pas", () => {
  ...
  const draft = { ...emptyBplEventDraft(), personName: "Jean Dupont", notes: "Preuve FI" };
...
test("FI(B) et FE(B) sont obligatoires selon le type", () => {
  assert.match(linkBplEventToAscension([], "TRAINING_FLIGHT_BPL", ascension("TRAINING_BPL"), emptyBplEventDraft()).error, /FI\(B\)/);
  assert.match(linkBplEventToAscension([], "PROFICIENCY_CHECK_BPL", ascension("PROFICIENCY_CHECK_BPL"), emptyBplEventDraft()).error, /FE\(B\)/);
});

test("associer un contrôle crée la preuve FE(B) correspondante", () => {
  const result = linkBplEventToAscension([], "PROFICIENCY_CHECK_BPL", ascension("PROFICIENCY_CHECK_BPL"), { ...emptyBplEventDraft(), personName: "Anne Martin" }, options);
...
test("modifier une preuve liée conserve UUID, lien, date du vol et marqueur de suppression", () => {
  const linked = linkBplEventToAscension([], "TRAINING_FLIGHT_BPL", ascension("TRAINING_BPL"), { ...emptyBplEventDraft(), personName: "FI Initial" }, options);
