...
export function savePilotProfile(profile: PilotProfile): PilotProfile { const normalized = normalizePilotProfile(profile); if (typeof window !== "undefined" && writeScopedBusinessValue(window.localStorage, STORAGE_KEY, JSON.stringify(normalized))) { enqueueLocalSyncMutation("pilot-profile", "singleton"); window.dispatchEvent(new Event(PILOT_PROFILE_EVENT)); } return normalized; }
