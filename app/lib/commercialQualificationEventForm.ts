...
export type CommercialEventDraft = Readonly<{ dateIso: string; classId: string; personName: string; theoryMinutes: string; trainingEventId: string; notes: string }>;
...
export function emptyCommercialEventDraft(event?: QualificationEvent): CommercialEventDraft {
  return { dateIso: event?.dateIso ?? "", classId: event?.balloonClass?.classId ?? "", personName: event?.examiner?.name ?? "", theoryMinutes: event?.theoryMinutes === undefined ? "" : String(event.theoryMinutes), trainingEventId: event?.relatedEventIds?.[0] ?? "", notes: event?.notes ?? "" };
}
...
export function upsertCommercialQualificationEvent(events: readonly QualificationEvent[], type: EditableCommercialEventType, draft: CommercialEventDraft, existingId?: string, options: Readonly<{ uuid?: () => string; now?: () => Date }> = {}): CommercialEventUpsertResult {
  ...
  if (type === "COMMERCIAL_REFRESHER_COURSE" && !events.some(({ id, type: eventType, instructor, balloonClass }) => id === draft.trainingEventId && eventType === "TRAINING_FLIGHT_BPL" && instructor?.name.trim() && balloonClass?.classId === draft.classId)) return { ok: false, error: "Le vol FI(B) doit correspondre à la même classe ballon." };
  const existing = existingId ? events.find(({ id, type: eventType }) => id === existingId && eventType === type) : undefined;
  const input = { type, dateIso: draft.dateIso, source: "MANUAL" as const, balloonClass: { classId: draft.classId }, ...(type === "COMMERCIAL_PROFICIENCY_CHECK" ? { examiner: { name: draft.personName.trim() } } : {}), ...(type === "COMMERCIAL_REFRESHER_COURSE" ? { theoryMinutes, relatedEventIds: [draft.trainingEventId] } : {}), ...(draft.notes.trim() ? { notes: draft.notes.trim() } : {}) };
