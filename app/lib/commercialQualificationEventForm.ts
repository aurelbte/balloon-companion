...
export type EditableCommercialEventType = "INITIAL_COMMERCIAL_ISSUANCE" | "COMMERCIAL_PROFICIENCY_CHECK" | "OPERATOR_PROFICIENCY_CHECK" | "COMMERCIAL_REFRESHER_COURSE";
export type CommercialEventDraft = Readonly<{ dateIso: string; classId: string; groupId: string; personName: string; theoryMinutes: string; trainingEventId: string; commercialQualifiedFiB: boolean; notes: string }>;
...
export function emptyCommercialEventDraft(event?: QualificationEvent): CommercialEventDraft {
  return { dateIso: event?.dateIso ?? "", classId: event?.balloonClass?.classId ?? "", groupId: event?.balloonClass?.groupId ?? "", personName: event?.examiner?.name ?? "", theoryMinutes: event?.theoryMinutes === undefined ? "" : String(event.theoryMinutes), trainingEventId: event?.relatedEventIds?.[0] ?? "", commercialQualifiedFiB: event?.commercialQualifiedFiB === true, notes: event?.notes ?? "" };
}
...
export function upsertCommercialQualificationEvent(events: readonly QualificationEvent[], type: EditableCommercialEventType, draft: CommercialEventDraft, existingId?: string, options: Readonly<{ uuid?: () => string; now?: () => Date }> = {}): CommercialEventUpsertResult {
  ...
  if ((type === "COMMERCIAL_PROFICIENCY_CHECK" || type === "OPERATOR_PROFICIENCY_CHECK") && !draft.personName.trim()) return { ok: false, error: "Renseignez l’examinateur." };
  ...
  const input = { type, dateIso: draft.dateIso, source: "MANUAL" as const, balloonClass: { classId: draft.classId, ...(draft.classId === "HOT_AIR_BALLOON" ? { groupId: draft.groupId } : {}) }, ...((type === "COMMERCIAL_PROFICIENCY_CHECK" || type === "OPERATOR_PROFICIENCY_CHECK") ? { examiner: { name: draft.personName.trim() } } : {}), ...(type === "COMMERCIAL_REFRESHER_COURSE" ? { theoryMinutes, relatedEventIds: [draft.trainingEventId], commercialQualifiedFiB: draft.commercialQualifiedFiB } : {}), ...(draft.notes.trim() ? { notes: draft.notes.trim() } : {}) };
