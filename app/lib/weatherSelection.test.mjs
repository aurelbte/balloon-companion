...
test("formate l'âge depuis le vrai timestamp", () => {
  ...
  assert.match(relativeUpdateLabel("2026-08-12T12:00:00Z", now), /^Données récupérées il y a 0 min/);
  assert.match(relativeUpdateLabel("2026-08-12T11:59:00Z", now), /^Données récupérées il y a 1 min/);
  assert.match(relativeUpdateLabel("2026-08-12T10:00:00Z", now), /^Données récupérées il y a 120 min ·/);
});
