...
export const REGISTERED_BALLOONS: readonly Balloon[] = [
  { id: "F-HLFM", registration: "F-HLFM", manufacturer: "Cameron", model: "Z105", category: "Libre à air chaud", volumeM3: 2_973, isFavorite: true, documents: [], weights: { fullCylinders: [] } },
...
export function createBalloon(input: BalloonInput, id = normalizeBalloonRegistration(input.registration)): Balloon {
  return { id, registration: normalizeBalloonRegistration(input.registration), manufacturer: input.manufacturer.trim(), model: input.model.trim(), category: input.category, volumeM3: input.volumeM3, ...(input.color?.trim() ? { color: input.color.trim() } : {}), documents: [], weights: { ...input.weights, fullCylinders: input.weights.fullCylinders.map((cylinder) => ({ ...cylinder, ...(cylinder.label?.trim() ? { label: cylinder.label.trim() } : {}) })) } };
}
