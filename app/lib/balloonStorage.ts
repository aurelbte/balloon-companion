...
export function loadBalloonRegistry(): BalloonRegistry { if (typeof window === "undefined") return createDefaultBalloonRegistry(); const empty = { version: BALLOON_REGISTRY_VERSION, balloons: [], activeBalloonId: null } satisfies BalloonRegistry; if (!getRuntimeDataScope()) return empty; try { const raw = readScopedBusinessValue(window.localStorage, STORAGE_KEY); if (!raw) return empty; return migrateBalloonRegistry(JSON.parse(raw)); } catch { return empty; } }
