...
export function loadBalloonRegistry(): BalloonRegistry { if (typeof window === "undefined") return createDefaultBalloonRegistry(); try { const parsed: unknown = JSON.parse(window.localStorage.getItem(STORAGE_KEY) ?? "null"); const registry = migrateBalloonRegistry(parsed); if (!parsed || typeof parsed !== "object" || (parsed as { version?: unknown }).version !== BALLOON_REGISTRY_VERSION) window.localStorage.setItem(STORAGE_KEY, JSON.stringify(registry)); return registry; } catch { return createDefaultBalloonRegistry(); } }
export function saveBalloonRegistry(registry: BalloonRegistry): void { if (typeof window === "undefined") return; window.localStorage.setItem(STORAGE_KEY, JSON.stringify(registry)); window.dispatchEvent(new Event(BALLOON_REGISTRY_EVENT)); }
