alter table public.logbook_entries
  add column if not exists regulatory_role text null,
  add column if not exists supervised_by_fi_b boolean null;

alter table public.logbook_entries
  drop constraint if exists logbook_entries_regulatory_role_check;

alter table public.logbook_entries
  add constraint logbook_entries_regulatory_role_check
  check (regulatory_role is null or regulatory_role in ('PIC', 'DUAL', 'FI_B', 'FE_B'));

create or replace function public.balloon_companion_apply_cloud_sync_mutation_logbook_entries(
  p_mutation_id uuid,
  p_entity_type text,
  p_entity_id text,
  p_operation text,
...
begin
  ...
  where key <> all (array[
    ...
    'takeoff_count', 'landing_count', 'instructor', 'examiner', 'regulatory_role', 'supervised_by_fi_b'
  ]) limit 1;
  ...
  if not found then
    ...
    insert into public.logbook_entries (
      ...
      nullif(p_payload->'examiner', 'null'::jsonb), nullif(p_payload->>'regulatory_role', ''),
      case when p_payload ? 'supervised_by_fi_b' then (p_payload->>'supervised_by_fi_b')::boolean else null end
    ...
    if p_operation = 'DELETE' then
      ...
      update public.logbook_entries t set
        ...
        regulatory_role = case when p_payload ? 'regulatory_role' then nullif(p_payload->>'regulatory_role', '') else t.regulatory_role end,
        supervised_by_fi_b = case when p_payload ? 'supervised_by_fi_b' then (p_payload->>'supervised_by_fi_b')::boolean else t.supervised_by_fi_b end
      where t.user_id = actor_id and t.id = p_entity_id
      returning t.revision, t.updated_at, t.deleted_at
        into result_revision, result_updated_at, result_deleted_at;
    end if;
...
revoke all on function public.balloon_companion_apply_cloud_sync_mutation_logbook_entries(uuid, text, text, text, bigint, jsonb) from public, anon, authenticated;
